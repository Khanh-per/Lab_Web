...
public class StudentController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (action) {
            ...
            // 👇 MERGE THESE CASES 👇
            case "search":
            case "filter": // Added this
            case "sort":   // Added this
            case "list":
            default:
    ...
    private void listStudents(HttpServletRequest request, HttpServletResponse response)
        ...
        // 1. Get ALL parameters sent by student-list.jsp
        String keyword = request.getParameter("keyword");
        String major = request.getParameter("major");
        String sortBy = request.getParameter("sortBy");
        String order = request.getParameter("order");

        // 2. Call the Master DAO method you already have in StudentDAO.java
        List<Student> students = studentDAO.searchAndFilterStudents(keyword, major, sortBy, order);

        // 3. Set attributes to preserve the state in the JSP
        request.setAttribute("students", students);
        request.setAttribute("keyword", keyword);
        request.setAttribute("selectedMajor", major); // Matches ${selectedMajor} in JSP
